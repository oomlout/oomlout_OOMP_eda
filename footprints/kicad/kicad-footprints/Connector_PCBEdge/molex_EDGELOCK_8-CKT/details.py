###### OOMP FILE  ######

import OOMP

newPart = OOMP.oompItem()

newPart.addTag("hexID", "")
newPart.addTag("oompType", "FOOTPRINT")
newPart.addTag("oompSize", "kicad")
newPart.addTag("oompColor", "kicad-footprints")
newPart.addTag("oompDesc", "Connector_PCBEdge")
newPart.addTag("oompIndex", "molex_EDGELOCK_8-CKT")
newPart.addTag("oompName", "kicad-footprints/Connector_PCBEdge/molex_EDGELOCK_8-CKT")

newPart.addTag("kicadDesc", "https://www.molex.com/pdm_docs/sd/2008900106_sd.pdf")
newPart.addTag("kicadTags", "Connector PCBEdge molex EDGELOCK")
newPart.addTag("kicadAttr", "exclude_from_pos_files exclude_from_bom")
newPart.addTag("kicad3DModel", "${KICAD6_3DMODEL_DIR}/Connector_PCBEdge.3dshapes/molex_EDGELOCK_8-CKT.wrl")

OOMP.parts.append(newPart)